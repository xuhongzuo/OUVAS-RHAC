...
public class Evaluation {
    ...
    public double computePresion(List listOfclass, LinkedHashMap<Integer,Integer> rankList) {  	
    	int nOutliers = 0;
    	int nObj = listOfclass.size();
    	double presion = 0.0;
    	...
    	for(int i = 0; i < listOfclass.size(); i++) {
    		if(listOfclass.get(i).equals(outlierClassName)){
    			int rank = rankList.get(i);
    			if(rank >= nObj - nOutliers) {
    				pNum++;
    			}
    		}
    	}
    	
    	presion = (double)pNum / (double)nOutliers;
